...
// Helper: Small async delay for backoff retry
...
`;
        ...
        for (const modelName of candidateModels) {
          try {
            ...
            const is503OrRateLimit = modelErr?.status === 503 || modelErr?.status === 429 || String(modelErr).includes("503") || String(modelErr).includes("high demand");
            if (is503OrRateLimit) {
              console.log(`[Gemini API Notice] Model ${modelName} temporarily busy, retrying fallback candidate...`);
              await sleep(250);
            } else {
              console.warn(`[Gemini API] Verification notice for ${modelName}:`, modelErr?.message || modelErr);
            }
          }
        }
...
5. Return strictly valid JSON adhering to the provided schema.`;
      ...
      for (const modelName of modelsToTry) {
        try {
          ...
          const is503OrRateLimit = modelErr?.status === 503 || modelErr?.status === 429 || String(modelErr).includes("503") || String(modelErr).includes("high demand");
          if (is503OrRateLimit) {
            console.log(`[Gemini API Notice] Model ${modelName} spike in demand, retrying fallback candidate...`);
            await sleep(250);
          } else {
            console.warn(`[Gemini API] Breakdown notice for ${modelName}:`, modelErr?.message || modelErr);
          }
        }
      }
    }
