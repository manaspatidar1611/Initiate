...
import { FlutterGuideModal } from './components/FlutterGuideModal';
...
export function App() {
  const [tasks, setTasks] = useState<Task[]>(() => {
    const saved = localStorage.getItem('initiate_tasks');
    if (saved) {
      try {
  ...
  const [isFlutterGuideOpen, setIsFlutterGuideOpen] = useState(false);

  // Save changes to localStorage
  useEffect(() => {
    localStorage.setItem('initiate_tasks', JSON.stringify(tasks));
  }, [tasks]);
    ...
    <DeviceFrame isFrameActive={isDeviceFrame}>
      <div className="min-h-full flex flex-col bg-[#F8FAFC] text-[#0F172A]">
        ...
        <Header
          ...
          onOpenFlutterCode={() => setIsFlutterGuideOpen(true)}
        ...
        {/* Flutter Code Reference Modal */}
        <FlutterGuideModal
          isOpen={isFlutterGuideOpen}
          onClose={() => setIsFlutterGuideOpen(false)}
        />
      </div>
    </DeviceFrame>
  );
